...
STANDALONE_NDK_HOME=temp-standalone-ndk
mkdir ${STANDALONE_NDK_HOME}

create_ndk() {
    echo "($1)..."
    "$MAKER" --unified-headers --api "$2" --arch "$1" --install-dir "${STANDALONE_NDK_HOME}/$1"
}
...
mkdir .cargo
#sed 's|$PWD|'"${PWD}"'|g' cargo-config.toml.template > .cargo/cargo-config.toml
